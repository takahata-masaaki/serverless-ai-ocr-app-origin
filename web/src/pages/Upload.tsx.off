...
const Upload: React.FC = () => {
  ...
  const selectedApp = apps.find((a: any) => a.name === appName);
  ...
  const [selectedEngine, setSelectedEngine] = useState<'yomitoku_ec2' | 'azure'>('azure');
  ...
  const refreshFiles = async () => {
    try {
      const res = await fetch(
        `${STARTER_API}/images?app_name=${encodeURIComponent(appName || '')}`,
        { method: 'GET' }
      );
  ...
  const handleSelectFiles = (list: FileList | null) => {
    ...
    setError(null);
    setFiles(arr);
    console.log('🔥 files set', arr.map(f => ({ name: f.name, type: f.type, size: f.size })));
  };
  ...
  const handleStartOcr = async () => {
    ...
    try {
      ...
      console.log('🔥 OCR start payload', payload);

      await fetch(`/ocr/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md">
        <div className="p-6 border-b border-gray-200">
          ...
          <div
            className="border-2 border-dashed border-gray-300 rounded-lg p-10 text-center mb-6"
          ...
          <div className="border rounded-lg p-4 mb-6">
            ...
            <label className="block mb-2">
              <input
                ...
                onChange={() => setPageProcessingMode('combined')}
                className="mr-2"
              />
              全ページ統合処理
            </label>
            ...
            <label className="block">
              <input
                ...
                onChange={() => setPageProcessingMode('individual')}
                className="mr-2"
              />
              ページ別個別処理
            </label>
      ...
      <S3SyncModal
        isOpen={showS3Sync}
        onClose={() => setShowS3Sync(false)}
        appName={appName || ''}
        onImportComplete={refreshFiles}
      />
      ...
      <LoadingToast
        show={uploading || isProcessing}
        message={uploading ? 'アップロード中...' : 'OCR処理中...'}
      />
    </div>
  );
};

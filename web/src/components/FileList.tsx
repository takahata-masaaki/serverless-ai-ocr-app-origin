          ...
          <div className="space-y-2">
            ...
            {mergedFiles.map(({ type, file }) => {
              if (type === 'parent') {
                ...
                return (
                  <div key={file.image_id} className="border border-gray-200 rounded-lg">
                    {/* 親ドキュメント行 */}
                    <div 
                      className="flex items-center p-4 cursor-pointer hover:bg-gray-50"
                      onClick={() => toggleParentExpansion(file.image_id)}
                    >
                      {/* アイコンエリア: 固定幅 */}
                  ...
                  {isExpanded && children.length > 0 && (
                    <div className="border-t border-gray-100">
                      {children.map((childFile) => (
                        <div key={childFile.image_id} className="flex items-center p-4 pl-12 hover:bg-gray-50">
                          {/* ページアイコン */}
                          ...
                          <div className="text-sm w-20 flex-shrink-0">
                            {childFile.status === 'completed' ? (
                              <button 
                                onClick={() => handleViewResult(childFile.image_id)} 
                ...
                // 通常ファイル（統合処理・既存データ）
                return (
                  <div key={file.image_id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center">
                      {/* アイコンエリア: 固定幅 */}
                      ...
                      <div className="text-sm w-20 flex-shrink-0">
                        {file.status === 'completed' ? (
                          <button 
                            onClick={() => handleViewResult(file.image_id)} 
                            className="text-blue-600 hover:text-blue-900"
                          >
                            結果表示
                          </button>
                      ...
                      <div className="w-8 flex-shrink-0 flex justify-center">
                        <button
                          onClick={() => handleDeleteClick(file.image_id, file.name)}
                          className="text-gray-400 hover:text-gray-600"
                          title="削除"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
